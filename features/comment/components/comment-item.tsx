...
import PostCardAuthor from "@/features/post/components/post-card-author";
        ...
        <div className="flex-1 space-y-2">
          <div className="flex items-center gap-2">
            ...
            {bet && (
              <Badge
                variant="secondary"
                className="flex items-center gap-1.5 text-xs font-medium px-2.5 py-0.5 bg-gradient-to-r from-yellow-500/10 to-orange-500/10 text-yellow-600 dark:text-yellow-400 border-yellow-200/50 dark:border-yellow-800/50 shadow-sm"
              >
                <Trophy className="w-3.5 h-3.5 text-yellow-500" />
                <span>
                  Bet: <span className="font-bold">{bet.target_winner_id}</span>
                  <span className="ml-1 opacity-75">
                    ({Number(bet.bet_amount)} pts)
                  </span>
                </span>
              </Badge>
            )}
